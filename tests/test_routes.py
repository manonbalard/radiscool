    ...
    with test_client as c:
        ...
        data = {
            ...
            "ingredients": json.dumps(
                [{"name_ingredient": "Flour", "quantity": 2, "unit": "cups"}]
            ),
        }
        files = {"recipeImage": (BytesIO(b"test image"), "test.jpg")}
        response = c.post(
            url_for("recipes.addrecipe_with_ingredients"),
            data=data,
...
def test_view_recipe_route(test_client, user):
    with test_client as c:
        login_user(user)
...
def test_edit_recipe_route(test_client, user):
    with test_client as c:
        login_user(user)
        recipe_id = 13  # Assurez-vous que l'ID est correct
        data = {
        ...
        response = c.post(
            url_for("recipes.edit_recipe_route", id=recipe_id),
            data=data,
            follow_redirects=True,
        )
        assert response.status_code == 200
...
def test_delete_recipe_route(test_client, user):
    with test_client as c:
        login_user(user)
        recipe_id = 13  # Assurez-vous que l'ID est correct
        response = c.post(
            url_for("recipes.delete_recipe_route", id=recipe_id), follow_redirects=True
        )
        assert response.status_code == 200
        assert b"La recette a ete supprimee avec succes." in response.data


def test_add_ingredient_route(test_client, user):
    with test_client as c:
        login_user(user)
        ...
        response = c.post(
            url_for("recipes.add_ingredient_route", recipe_id=recipe_id),
            data=data,
            follow_redirects=True,
        )
        assert response.status_code == 200
...
def test_delete_ingredient_route(test_client, user):
    with test_client as c:
        login_user(user)
        ...
        response = c.post(
            url_for(
                "recipes.delete_ingredient_route",
                recipe_id=recipe_id,
...
def test_add_comment_route(test_client, user):
    with test_client as c:
        login_user(user)
        recipe_id = 8  # Assurez-vous que l'ID est correct
        data = {"comment": "This is a test comment"}
        response = c.post(
            url_for("recipes.add_comment_route", recipe_id=recipe_id),
            data=data,
            follow_redirects=True,
        )
...
def test_rate_recipe_route(test_client, user):
    with test_client as c:
        login_user(user)
        recipe_id = 15  # Assurez-vous que l'ID est correct
        data = {"stars": 5}
        response = c.post(
            url_for("recipes.rate_recipe_route", recipe_id=recipe_id),
            data=data,
            follow_redirects=True,
        )
        assert response.status_code == 200
