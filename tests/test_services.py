...
from services.recipe_service import add_recipe, delete_recipe, edit_recipe, rate_recipe
...
def test_add_ingredient_to_recipe(mock_recipe_ingredient, mock_ingredient, mock_db):
    ...
    result = add_ingredient_to_recipe(recipe_id=1, name_ingredient="Salt", quantity=1, unit="tsp")
    assert result['error'] is False
...
def test_delete_ingredient(mock_recipe_ingredient, mock_db):
    mock_recipe_ingredient.query.filter_by.return_value.first_or_404.return_value = MagicMock()
    mock_db.commit = MagicMock()
